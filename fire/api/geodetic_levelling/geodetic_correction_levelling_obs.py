...
def apply_geodetic_corrections_to_height_diffs(
    ...
    The metric height differences of a FIRE project are converted to geopotential units if and only if
    the function is called with arguments for both parameter gravitymodel and grid_inputfolder.
    ...
    differences, only "non" for non-tidal supported so far
    ...
    for index in observations_df.index:
        ...
        # the function apply_geodetic_corrections_to_height_diffs is called with arguments for
        # both parameter gravitymodel and grid_inputfolder
        if (gravitymodel is not None) and (grid_inputfolder is not None):
    ...
    parameters_new_df = pd.DataFrame(
        {
            "Navn": [
                ...
                "Target epoch",
                "Deformationmodel",
                "Gravitymodel for conversion to gpu",
            ],
            "Værdi": [tidal_system, epoch_target, deformationmodel, gravitymodel],
        },
    )
